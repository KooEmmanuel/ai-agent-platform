#!/usr/bin/env python3
"""
Test script to check Email Sender configuration
"""
...
async def test_email_sender_config():
    """Test getting Email Sender configuration"""
    db = await anext(get_db())
    
    try:
        print("=== Testing Email Sender Configuration ===")
        
        # Test with Email Sender (ID: 17) and Agent ID: 1
        tool_id = 17
        agent_id = 1
        
        print(f"Testing tool ID: {tool_id} (Email Sender)")
...
if __name__ == "__main__":
    asyncio.run(test_email_sender_config()) 